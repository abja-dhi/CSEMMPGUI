...
class ADCPMainWindow(QMainWindow):
    ...
    def __init__(self, adcps):
        ...
        self.variable_selector.currentTextChanged.connect(self.change_variable)
    ...
    def change_variable(self, variable_name):
        ...
        all_values = np.concatenate(self.canvas.backscatter_cache)
        self.canvas.norm = Normalize(vmin=all_values.min(), vmax=all_values.max())
        for i, lc in enumerate(self.canvas.collections):
            if i != self.canvas.selected_index and i != self.canvas.hover_index:
