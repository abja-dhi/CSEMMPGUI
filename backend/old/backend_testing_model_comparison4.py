# ============================== MIXED MT–HD TRANSECT PLOT (SSC map + currents + metadata) ==============================
# Top: MT SSC raster over bbox, ADCP track colored by SSC, ADCP quivers colored by SSC, Model quivers single color (or field mode)
# Bottom: metadata block
# Assumes: mt_model, hd_model, adcp, crs_helper already constructed as below
...
adcp_cfg["velocity_average_window_len"] = 10  # user-chosen smoothing
...
# ============================== INPUTS (edit here) ==============================
# ----- SSC map options -----
mt_item_number        = 1                      # MT item for SSC
scale                 = "log"                  # "log" | "normal"
levels                = [0.01, 0.1, 1, 10, 100]# mg/L colorbar ticks
vmin, vmax            = None, None             # mg/L; None -> auto from levels/data
cmap_name             = "turbo"                # or cmo.cmap e.g., cmo.turbid
cmap_bottom_thresh    = 0.01                   # mg/L; transparent below threshold
pixel_size_m          = 10                     # MT raster resolution
pad_m                 = 2000                   # bbox padding

tick_decimals         = 2                      # colorbar tick text precision
tick_decimal_precision= 3                      # axis tick precision for map axes

# ----- Currents (HD) -----
u_item_number         = 4                      # HD U item
v_item_number         = 5                      # HD V item
model_quiver_mode     = "field"             # "transect" | "field"
quiver_every_n        = 20                     # downsample quiver along track
quiver_scale          = 3                     # quiver scale for both layers
quiver_color_model    = "black"                # model quiver color (single-color)
adcp_transect_lw      = 1.8                    # line width for colored ADCP track
field_pixel_size_m    = 100                    # coarse field resolution (field mode)
field_quiver_stride_n = 3                      # stride for field vectors

# ----- ADCP aggregation for SSC coloring and vectors -----
adcp_series_mode      = "bin"                  # "bin" | "range" | "hab"
adcp_series_target    = "mean"                 # numeric value or "mean"/"pXX"
...
# bring right column in and keep constant spacing
META_LEFT_OVERSHOOT = 0.0
META_COL_START      = 0.02
META_COL_END        = 0.7          # was 0.84
META_COLS           = 3
...
# ====================================================================== TOP PANEL — MT SSC raster + ADCP/Model vectors
# ----- Rasterize SSC from MT -----
...
cmap = cmap.copy(); cmap.set_under(alpha=0.0)
...
# ----- Colorbar (flush to right with gap) -----
fig.canvas.draw()
...
if _ticks:
    ...
    _cb = np.geomspace(cbar_min, cbar_max, 5) if scale.lower() == "log" else np.linspace(cbar_min, cbar_max, 5)
    cb.set_ticks(_cb); cb.set_ticklabels([_fmt_num(v, tick_decimals) for v in _cb])

# ----- Legend (static handlers to avoid sampling data again) -----
from matplotlib.patches import FancyArrowPatch
from matplotlib.legend_handler import HandlerPatch
from matplotlib.lines import Line2D
...
EDGE_LW = 0.05   # outline thickness
TAIL_W  = 0.1    # shaft width (rel. to mutation_scale)
HEAD_W  = 0.5    # head width  (rel. to mutation_scale)
HEAD_L  = 0.5    # head length (rel. to mutation_scale)

# Mean SSC color for ADCP legend arrow and track line
if 'C_adcp' in locals() and np.isfinite(C_adcp).any():
    _mean_ssc_leg = float(np.nanmean(C_adcp))
else:
    _mean_ssc_leg = 0.5 * (cbar_min + cbar_max)
adcp_mean_rgba = cmap(norm(_mean_ssc_leg))

# On-map thin overlay of the ADCP track in this same color
...
def _legend_arrow(legend, orig_handle, xdescent, ydescent, width, height, fontsize):
    ...
    # optional subtle edge boost while keeping thin outline
...
model_ssc_ts = np.asarray(mt_model.rasterize_idw_bbox(item_number=mt_item_number, bbox=bbox, t=t, pixel_size_m=pixel_size_m)[0], float).ravel() * 1000.0
...
# ----- Scalars -----
t0 = pd.to_datetime(t_arr.min()); t1 = pd.to_datetime(t_arr.max())
...
def I(x, y, k, v): ax2.text(x, y, f"{k}: {v}", ha="left", va="top", fontsize=7, family="monospace")
...
# Column 1 — Observation aggregation + vector mode
x = cols_x[0]; y = y0
H(x, y, "Observation Aggregation"); y -= sec
I(x, y, "SSC mode", adcp_series_mode); y -= line
I(x, y, "SSC target", adcp_series_target); y -= line
...
x = cols_x[2]; y = y0
H(x, y, "Summary"); y -= sec
I(x, y, "Mean SSC (obs)",   f"{mean_ssc_obs:.{tick_decimals}f} mg/L"); y -= line
I(x, y, "Mean SSC (model)", f"{mean_ssc_model:.{tick_decimals}f} mg/L"); y -= line
I(x, y, "Mean SSC error",   f"{mean_ssc_err:.{tick_decimals}f} mg/L"); y -= line
