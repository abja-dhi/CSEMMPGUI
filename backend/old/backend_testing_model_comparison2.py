...
# metadata spacing
META_TOP_Y       = 0.95               # start y in axes coords
META_SECTION_GAP = 0.20               # gap after each header
META_LINE_GAP    = 0.16               # gap between info lines
META_COL_X       = [0.02, 0.48, 0.84] # increased horizontal spacing
META_LEFT_OVERSHOOT = 0.1            # push bottom panel further left (figure fraction)
...
fig.canvas.draw()
p0 = ax0.get_position()           # top panel box (after CB)
p1 = ax1.get_position()
...
# y-limit: set to match colorbar max
if scale.lower() == "log":
    ax1.set_yscale("log")
...
new_w  = min(p0.width + META_LEFT_OVERSHOOT, 1.0 - new_x0)  # keep right edge ≤ 1
...
# Column 3
x = cols_x[2]; y = y0
H(x, y, "Model Fit"); y -= sec
I(x, y, "Mean error",     f"{mean_err:.{tick_decimals}f} mg/L"); y -= line
I(x, y, "Mean observed",  f"{mean_obs:.{tick_decimals}f} mg/L"); y -= line
