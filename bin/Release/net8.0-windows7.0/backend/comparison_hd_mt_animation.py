...
def make_ssc_currents_animation(
    ...
    if time_end_idx is None:
        time_end_idx = times_all.size
    sel_idx = np.arange(time_start_idx, time_end_idx, max(1, int(time_step)))
    ...
    if save_output:
        ...
        if _HAS_IMAGEIO:
            ...
            if writer_kind.lower() == "gif":
                imageio.mimsave(str(out_path), frames_rgba, duration=max(0.001, interval_ms / 1000.0))
            elif writer_kind.lower() == "mp4":
                imageio.mimsave(str(out_path), frames_rgba, fps=max(1, int(1000 / max(1, int(interval_ms)))))
            else:
                PROG.close()
            ...
            fps = max(1, int(1000 / max(1, int(interval_ms))))
...
if __name__ == "__main__":
    ...
    fig, ani = make_ssc_currents_animation(
        ...
        interval_ms=.1,                 # numericAnimationInterval        
        out_path="ssc_currents.gif",    # txtAnimationOutputFile and btnAnimationOutputFile
