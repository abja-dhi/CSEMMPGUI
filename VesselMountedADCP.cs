...
namespace CSEMMPGUI_v1
{
    public partial class VesselMountedADCP : Form
    {
        ...
        public int id = 0; // ID of the instrument
        XmlElement? instrument;
        public bool isSaved = true; // Track if survey has been saved

        public VesselMountedADCP(SurveyManager _surveyManager)
        {
            InitializeComponent();
            surveyManager = _surveyManager;
            id = surveyManager.NInstrument(type: "VesselMountedADCP") + 1;
            Initialize();
        }
        ...
        private void btnLoadPD0_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    { "Task", "LoadPD0" },
                    { "PD0Path", ofd.FileName },
                };

                string xmlInput = Tools.GenerateInput(inputs);
                XmlDocument result = Tools.CallPython(xmlInput);
                Dictionary<string, string> outputs = Tools.ParseOutput(result);
        ...
        private void btnLoadPosition_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (csvOptions.ShowDialog() == DialogResult.OK)
                {
                    ...
                    columns = Utils.ParseCSVAndReturnColumns(filePath, delimiter, headerLines);
                    if (columns.Length < 4)
                    {
